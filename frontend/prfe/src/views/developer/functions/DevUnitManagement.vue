      ...
      <div class="content">
        ...
        <div v-if="site" class="site-container">
          ...
          <div class="floors-section">
                  ...
                  <div class="toolbar">
                    <div class="left-section">
                      ...
                      <div class="floor-sort">
                        ...
                        <select id="sortFloors" v-model="floorSortOrder" @change="sortFloors" class="dropdown">
            ...
            <div v-if="sortedFloors.length > 0" class="floor-table">
              <div class="outside-headers">
                <span class="header-item">Floor #</span>
                <span class="header-item">Total Units</span>
                <span class="header-item">Available Units</span>
                <span class="header-item">Actions</span>
              </div>

              <div v-for="floor in sortedFloors" :key="floor.id" class="card border-0 rounded-1 mx-auto my-2" style="
            ...
            <div class="floor-sort">
              <label for="sortFloors">Sort Floors:</label>
              <select id="sortFloors" v-model="floorSortOrder" @change="sortFloors">
                <option value="asc">Ascending</option>
                <option value="desc">Descending</option>
              </select>
            </div>
            <div v-if="sortedFloors.length > 0" class="floor-list">
              <div v-for="floor in sortedFloors" :key="floor.id" class="floor-card">
        ...
        <b-modal id="add-units-modal" title="Add Units" v-model="showAddUnitsModal" :disabled="isSaveButtonDisabled"
          ok-title="Save" @ok="addUnits">
          <form @submit.prevent="addUnits">
            <!-- Floor Selection (Multiple Selection) -->
            <b-form-group label="Floors:" description="Select floors to add units to">
              <b-form-select v-model="newUnitFloors" :options="floorOptions" required multiple></b-form-select>
            </b-form-group>
            ...
            <!-- Unit Type -->
            <b-form-group label="Unit Type:" description="Select the type of unit">
            ...
            <!-- Price -->
            <b-form-group label="Price:" description="Enter the price of the unit">
            ...
            <b-form-group label="Commission:">
              <b-form-input type="number" v-model.number="newUnitCommission" min="0" required></b-form-input>
            </b-form-group>

            <!-- Spot Discount Percentage -->
            <b-form-group label="Spot Discount Percentage:">
              <b-form-input type="number" v-model.number="newUnitSpotDiscountPercentage" min="0"></b-form-input>
            </b-form-group>
              ...
              <b-form-group label="Unit Images:">
                <div v-if="selectedUnit.images && selectedUnit.images.length">
                  <b-row class="mb-3">
                    ...
                    <b-col v-for="(image, index) in selectedUnit.images" :key="index" cols="12" sm="6" md="4" lg="3"
                      class="mb-2">
                      <div class="d-flex flex-column align-items-center">
                        <b-img v-if="image && image.image" :src="getPictureUrl(image.image)" alt="Unit Image" thumbnail
                          fluid class="unit-image-preview" />
                        ...
                        <div class="text-center mt-2">
                          ...
                          <b-button size="sm" variant="primary" class="ml-2" @click="toggleImageEdit(index)">
                            Update
                          </b-button>
                          <b-button size="sm" variant="danger" class="ml-2" @click="deleteImage(index)">
                            Delete
                          </b-button>
                        </div>
                        ...
                        <div v-if="imageEditIndex === index" class="mt-2">
                          <input type="file" accept="image/*" @change="onImageSelected(index, $event)" />
                          <b-button variant="secondary" @click="replaceImage(index)">
                            Replace Image
                          </b-button>
                        </div>
                      </div>
                ...
                <div v-if="selectedUnit.images && selectedUnit.images.length < 5" class="mt-3">
        ...
        <b-modal id="add-floor-units-modal" title="Add Units to Floor" v-model="showAddFloorUnitsModal" ok-title="Save"
          @ok="addFloorUnits(newUnitFloors[0])">
          <form @submit.prevent="addFloorUnits(newUnitFloors[0])">
            <!-- Only one floor (already set) -->
            <b-form-group label="Floor:" description="This floor will have new units added.">
              <b-form-select v-model="newUnitFloors" :options="floorOptions" required disabled></b-form-select>
            </b-form-group>
...
.floor-info th:nth-child(2),
.floor-info td:nth-child(2) {
  /* Location column */
  width: 20%;
}
