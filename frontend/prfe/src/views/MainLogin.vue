  ...
  <section class="hero overlay">
    <div class="container h-100">
      <div class="row d-flex justify-content-center align-items-center h-100">
        <div class="col-md-8 col-lg-6 col-xl-5"> <!-- Ensure responsive centering -->
          <div class="card rounded-3 text-black">
            <div class="card-body p-md-5 mx-md-4">
              <form @submit.prevent="login">
                <br />
                <h2 class="text-start">Hello!</h2>
  ...
  methods: {
    ...
    async login() {
      ...
      if (this.username && this.password) {
        try {
          // Attempt to authenticate as a broker first
          ...
          if (response.ok && data.tokens) {
            ...
            // If broker login fails, try developer login
            response = await fetch("http://localhost:8000/api/token/developer/", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
    ...
    // Factor out the common login handling logic
...
<style>
.overlay {
  background: linear-gradient(to bottom, rgba(113, 113, 113, 0.4), rgba(44, 171, 255, 0.7));
}
.hero {
